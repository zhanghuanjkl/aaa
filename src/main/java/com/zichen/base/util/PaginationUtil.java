...
public class PaginationUtil implements Serializable {
    ...
    public String getPageHtml() {
        ...
        stb.append("<div class='ejpagination-pages fr'>");
        if (num != 1) {
            stb.append("<a href='" + (url + 1)
                       + "'><span class='page-first'>首页</span></a><a href='" + (url + prev)
                       + "'><span class='page_next'>上一页</span></a>");
        } else {
            stb.append("<span class='page-first'>首页</span> ");
        }
        ...
        if (beginPage <= pageCount) {
            ...
            for (int j = 0; (i <= pageCount) && (j < 6); j++) {
                if (num != i) {
                    stb.append("<a href='" + (url + i) + "'><span class='page-wrap'>" + i
                               + "</span></a>");
                } else {
                    stb.append("<span class='current disabled'>" + i + "</span>");
                }
                i++;
            }
        }
        ...
        if (num != 0 && pageCount != 0 && num != pageCount) {
            stb.append("<a href='" + (url + next)
                       + "'><span class='page_next'>下一页</span></a><a href='" + (url + last)
                       + "'><span class='page_next'>尾页</span></a>");
        } else {
            stb.append("<span class='page_next'>尾页</span> ");
        }
        stb.append("<span class='page_next'>共" + pageCount + "页</span> ");
        stb.append("</div>");
