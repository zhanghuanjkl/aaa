...
public class DateUtil {
    ...
    // 格式：年－月－日 小时：分钟：秒(中间没有分割符号)
    public static final String yyyyMMddHHmmss_NO = "yyyyMMddHHmmss";
   ...
   /**
    * 系统时间加上 确定天数后的时间===提前日期
    */
    public static Date getAdvanceDate(Date currDate, Integer advanceDays) {
    	        Calendar calendar = Calendar.getInstance();
    	        calendar.setTime(currDate);
    	        calendar.set(Calendar.DATE, calendar.get(Calendar.DATE)+advanceDays);
    	        Date compareDate=new Date();
    	        try {
					 compareDate= timeFormat.parse(timeFormat.format(calendar.getTime()));
